...
static char *
duo_remember_file(struct passwd *pw, const char *fn)
{
...
duo_is_remembered(struct passwd *pw, const char *fn, struct duo_config *cfg, const char *host)
{
    ...
    if (rmfile != NULL && cfg->remember_me_duration > 0) {
        ...
        while ((fread(&remem, sizeof(remem), 1, rmfile) > 0) && (is_remembered == 0)) {
            is_remembered = ((memcmp(remem.addr, addr, sizeof(addr)) == 0) && (difftime(now, remem.time) < (double) cfg->remember_me_duration));
        }
    }
    if (rmfile != NULL) {
        fclose(rmfile);
...
duo_remember(struct passwd *pw, const char *fn, struct duo_config *cfg, const char *host)
{
    if (cfg->remember_me_duration > 0) {
        ...
        while ((fread(&remem, sizeof(remem), 1, rmfile) > 0) && (difftime(now, remem.time) < (double) cfg->remember_me_duration)) {
            skip_count++;
        }
